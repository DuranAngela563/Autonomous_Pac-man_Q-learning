    ...
    def run( self ):
        ...
        while not self.gameOver:
            ...
            if agentIndex == 0:
                ...
                ######################################################
                '''
                state_tick = agent.getState(observation)
                # lin 694 action = agent.getAction(observation)
                action = agent.getAction(state_tick)
                #action = agent.getAction(observation)

                state_next_tick = agent.getState(self.state)
                reward = self.getReward(self, state_tick, action, state_next_tick)

                agent.update(state_tick, action, state_next_tick, agent.getReward(state_tick, action, state_next_tick))
                '''
